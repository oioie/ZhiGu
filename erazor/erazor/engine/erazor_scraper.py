...
class Scraper:
    ...
    def _process_spidermw_output(self, output: Any, request: Request, response: Response,
        ...
        if isinstance(output, Request):
            ...
            return dfd
        elif isinstance(output, SpiderTask):
            spider.logger.info(f'正在推送任务==> [task_id:{output.task_id}]')
            # task_json = output.to_json()
            # spider.TaskQueue.push(task_json.get("priority"), task_json)
            spider.task_manager.push_task(priority=0, task=output)
        elif output is None:
            pass
    ...
    def _itemproc_finished(self, output: Any, item: Any, response: Response, spider: Spider) -> None:
        ...
        if isinstance(output, Failure):
            ...
            saved_count = response.meta.get('saved_count', 0)
            saved_count += output.get("saved_count", 0)
            response.meta['saved_count'] = saved_count
            save_failure_count = response.meta.get('save_failure_count', 0)
            save_failure_count += output.get("save_failure_count", 0)
            response.meta['save_failure_count'] = save_failure_count
            if spider.custom_settings['MODE'] == 'STRICT' and saved_count == response.meta.get("all_items"):
                spider.task_manager.release_task(
                    priority=response.meta['task_body'].get('priority', 0),
                    task_id=response.meta['task_body'].get('task_id')
                )
            return self.signals.send_catch_log_deferred(
